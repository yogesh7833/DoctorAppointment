...
const Navbar = () => {
  ...
  return (
    <div className='flex items-center justify-between text-sm py-4 mb-5 border-b border-b-gray-400'>
        <Link to='/'><img className='w-44 cursor-pointer ' src={assets.quickcure}/></Link>
        ...
        <div className='flex items-center gap-4'>
            ...
            <div className={`${showMenu ? 'fixed w-full':'h-0 w-0'}  md:hidden right-0 top-0 bottom-0 z-20 overflow-hidden bg-white transition-all duration-300`}>
                <div className='flex items-center justify-between px-5 py-6'>
                    <img className='w-36 ' src={assets.quickcure}/>
                ...
                <ul className='flex flex-col items-center gap-2 mt-5 px-5 text-lg font-medium '>
                    ...
                    <NavLink  onClick={()=>setShowMenu(false)} to='/login'><p className='px-4 py-2 rounded inline-block'>Sign Up/ Login</p></NavLink>
                </ul>
            </div>
        </div>
    </div>
