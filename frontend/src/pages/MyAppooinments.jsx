...
const MyAppooinments = () => {
  ...
  const initPay = (order) =>{
   const options={
      ...
      descripiton:'Appointment Payment',
      order_id: order.id,
      ...
      //  }
      try {
        const { razorpay_order_id, razorpay_payment_id, razorpay_signature } = response; // Extract necessary data
...
//             <div>
//               <img className='w-32 bg-indigo-50' src={item.docData.image || '/path/to/placeholder.jpg'} alt="Doctor" />
//             </div>
//             <div className='flex-1 text-sm text-zinc-600'>
//               <p className='text-neutral-800 font-semibold'>{item.docData.name}</p>
//               <p>{item.docData.speciality}</p>
...
//                 <span className='text-sm text-neutral-700 font-medium'>Date & Time:</span>{slotDateFormate(item.slotDate)} | {item.slotTime}
//               </p>
//             </div>
//             <div></div>
//             <div className='flex flex-col gap-2 justify-end'>
